using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using Inventory.ItemData;

public class ItemPickUp : MonoBehaviour
{
    [field: SerializeField] public ItemInfo Item { get; private set; }
    [field: SerializeField] public int Quantity { get; private set; }
    [SerializeField] private AudioSource pickUpSound;
    [SerializeField] private float duration = 0.2f; // 아이템 오브젝트 파괴 지연 시간

    private void Start()
    {
        GetComponent<SpriteRenderer>().sprite = Item.Item_Image;
    }

    public void Item_Pickup(InventoryInfo inventory)
    {
        // 1) 인벤토리에 아이템 추가 요청
        int reminder = inventory.AddItem(Item, Quantity);
        if (reminder == 0)
        {
            DestroyItem();
        }
        else
        {
            Quantity = reminder; // 남은 수량 업데이트
        }
    }
    public void DestroyItem()
    {
        GetComponent<Collider2D>().enabled = false; // 충돌 비활성화
        StartCoroutine(AnimateItemPickup());
    }
    private IEnumerator AnimateItemPickup()
    {
        // 1) 획득 효과 재생 (사운드, 파티클)
        //pickUpSound.Play();

        // 아이템 오브젝트를 천천히 사라지게 함
        Vector3 startScale = transform.localScale;
        Vector3 endScale = Vector3.zero;
        float currentTime = 0;
        
        // 아이템 축소 애니메이션
        while (currentTime < duration)
        {
            currentTime += Time.deltaTime;
            transform.localScale = Vector3.Lerp(startScale, endScale, currentTime / duration);
            yield return null;
        }
        transform.localScale = endScale;

        // 3) 즉시 제거하여 리소스 최소화
        Destroy(gameObject);
    }
}
